...
def copy_n_samples(df, N, data_folder, output_folder, file_column, class_column, score_column, per_class=False, quality="top"):
    ...
    for i, (f, c) in enumerate(zip(files, class_labels)):
        ...
        if N == 1:
            out_file_name = join(output_folder, quality, quality + "_" + str(c) + splitext(f)[1])
        else:
            out_file_name = join(output_folder, quality, str(c) + str(i).zfill(2) + splitext(f)[1])
...
def plot_images_from_directory(directory_path, output_path, target_size=(100, 100)):
    ...
    for image_file in image_files:
        ...
        ab = AnnotationBbox(imagebox, (0, 0), frameon=False, xycoords='axes fraction', boxcoords="axes fraction", pad=0)
...
copy_n_samples(merged_df, 1, data_folder, image_out_dir, "Path", "decoded", "ranks", quality="top", per_class=True)
copy_n_samples(merged_df, 1, data_folder, image_out_dir, "Path", "decoded", "ranks", quality="bottom", per_class=True)
